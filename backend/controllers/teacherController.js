...
const getAllStudents = async (req, res) => {
  try {
    ...
    const existingStudentUserIds = students
      .filter((s) => s.userId)
      .map((s) => String(s.userId._id));

    const missingUsers = await User.find({
      role: 'student',
      _id: { $nin: existingStudentUserIds }
    }).select('name email').exec();

    const missingStudentRecords = missingUsers.map((user) => ({
      _id: user._id,
      userId: user,
      rollNo: 'N/A',
      department: 'N/A'
    ...
    res.json({ success: true, data: [...students, ...missingStudentRecords] });
  } catch (error) {
